...
class Patchify(torch.nn.Module):
    ...
    def __init__(self, sths=[1,3,5], fixed_length=196, cannys=[50, 100], patch_size=16, num_channels=3, physics = False, edge_percentage=.1) -> None:
        super().__init__()
    ...
    def forward(self, img):  # we assume inputs are always structured like this
        ...
        if self.physics:
            ...
            grad_mag = scipy.ndimage.filters.laplace(np.squeeze(img))
            ind = np.unravel_index(np.argsort(grad_mag, axis=None), grad_mag.shape)
            ...
            #H = scipy.ndimage.filters.laplace(np.squeeze(img))
            #edges
        else:
