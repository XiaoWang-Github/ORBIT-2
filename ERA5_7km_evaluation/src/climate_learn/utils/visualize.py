...
def clip_replace_input(x , in_variables):

    prcp_index = in_variables.index("total_precipitation_24hr")
    for i in range(x.shape[1]):
        if i==prcp_index:
            torch.clamp_(x[:,prcp_index,:,:], min=0.0)
...
def clip_replace_constant(y, yhat, out_variables):
    ...
    prcp_pred = yhat[:, prcp_index, :, :]
    prcp_pred[prcp_pred < 0] = 0.0
    yhat[:, prcp_index, :, :] = prcp_pred
...
def visualize_at_index(mm, dm, dm_vis, out_list, in_transform, out_transform,variable, src, device, div, overlap,index=0, tensor_par_size=1,tensor_par_group=None):
    ...
    print ("****************we are processing index", index)
    ...
    for vindex in range(div):
        for hindex in range(div):
            ...
            xdiv = clip_replace_input(xdiv , in_variables)
            xdiv = xdiv.to(device)
    ...
    plt.show()
    name = str(torch.distributed.get_rank())+f'_prediction_{index+1}.png'
    plt.savefig(name)
    #np.save(str(torch.distributed.get_rank())+'_preds.npy', preds )
    np.save(f'out_pred/preds_day_{int(index)//24 + 1}.npy', preds)
