...
def clip_replace_temp(y, yhat, in_variables, out_variable):

    land_sea_mask_idx = in_variables.index('land_sea_mask')
    land_sea_mask = in_variables[land_sea_mask_idx]

    sea_mask = (land_sea_mask == 0)

    modified_yhat = yhat

    modified_yhat[sea_mask] = y[sea_mask]

    return modified_yhat
...
def test_on_many_images(mm, dm, in_variables, out_variables, in_transform, out_transform, variable, src, outputdir, device, index=-1):
    ...
    for batch in dm.test_dataloader():
        ...
        #pred = clip_replace_temp(y, pred, in_variables, out_variables)
        ...
        ppred = clip_replace_temp(yy, ppred, in_variables, out_variables)
